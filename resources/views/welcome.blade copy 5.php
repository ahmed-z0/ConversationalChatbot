        ...
        if (sentences.length > 1) {
            ...
            sentences.slice(0, -1).forEach(sentence => {
                if (sentence.trim()) {
                    ...
                    processIncomingTexts(sentence.trim());
                }
            });
...
function processIncomingTexts(text) {
    ...
    const voiceId = "1J4RetdHwFoQxvr6VQT7"; // replace with your voice_id
...
socket.onopen = function (event) {
    ...
    "chunk_length_schedule": [120, 160, 250, 290]
        ...
        "xi_api_key": "409f724084beb97d6043ce8ea3d513f9", // replace with your API key
    };
...
socket.onmessage = function (event) {
    ...
    if (response.audio) {
        // Convert the base64 audio chunk to a Blob
        const audioBlob = base64ToBlob(response.audio);
        // Create an object URL for the Blob
        const audioUrl = URL.createObjectURL(audioBlob);
        // Create a new Audio object and play it
        const audio = new Audio(audioUrl);
        audio.play().then(() => console.log("Audio playing")).catch(e => console.error("Playback failed", e));
    ...
    if (response.normalizedAlignment) {
        ...
        console.log("Normalized alignment:", response.normalizedAlignment);
    }
};
